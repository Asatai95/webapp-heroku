...
def twitter_callback():
    ...
    data = get_twitter_request_token()
    if data:
        check = check_socials(data, 'twitter')
        if check is not False:
            social = session.query(User.name).join(
                     Social, User.id == Social.user_id).filter(
            ...
            user = create_twitter_user(data)
            create_socials(user, data, 'twitter')
            login_user(user.id)
            redirect('/mypage')
    else:
        redirect('/')
...
def tweer_db():
    ...
    current_user = get_current_user()
    tweet_comment = tweet_comment_create(request.forms)
    ...
    return template('templates/tweet', tweet_comment=tweet_comment, tweet_db=tweet_db, follow_view=follow_check , fab_check=fab_check,  tweets=tweets, img=img_table, current_user=current_user)
...
@route('/follower/delete/<delete_follower_id:int>')
...
def search():
    ...
    if search is not False and search != 'error':
        test=''
        tweets = search
